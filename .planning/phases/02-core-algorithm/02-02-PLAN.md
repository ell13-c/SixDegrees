...
def test_score_between_zero_and_one_for_high_counts():
    """INT-03: Pair with max interactions among all pairs should get score close to 1.0."""
    ids = ["u1", "u2"]
    raw = {canonical_pair("u1", "u2"): {"likes": 100, "comments": 100, "dms": 100}}
    matrix = compute_interaction_scores(ids, raw)
    # Only one pair — normalized to max → should be 1.0
    assert abs(matrix[0][1] - 1.0) < 1e-6, f"Expected ~1.0, got {matrix[0][1]}"
